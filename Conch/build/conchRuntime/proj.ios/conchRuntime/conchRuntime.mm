...
-(id)initWithView:(GLKView*)pGLView frame:(CGRect)frame EAGLContext:(EAGLContext*) pGLContext downloadThreadNum:(int)nDownloadThreadNum URL:(NSString*)pUrl
{
    ...
    if( self != nil )
    {
        ...
        m_initFrame = m_currentFrame = frame;
...
{
    self = [self initWithView:pGLView frame:frame EAGLContext:pGLContext downloadThreadNum:nDownloadThreadNum URL:nil];
    if( self != nil )
    {
        return self;
    }
...
{
    // 1.检测wifi状态
    LayaReachability *wifi = [LayaReachability reachabilityForLocalWiFi];
...
-(void) setScreenOrientation:(int)p_nType
{
    ...
    switch(p_nType)
    {
        case 0://landscape 16
        {
            m_currentFrame.size.width = MAX( kRect.size.width , kRect.size.height );
            m_currentFrame.size.height = MIN( kRect.size.width , kRect.size.height );
        ...
        case 1://portrait 2
        {
            m_currentFrame.size.width = MIN( kRect.size.width , kRect.size.height );
            m_currentFrame.size.height = MAX( kRect.size.width , kRect.size.height );
