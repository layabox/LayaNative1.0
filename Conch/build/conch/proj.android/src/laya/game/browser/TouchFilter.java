...
public class TouchFilter
{
	...
	final static char code[]="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ+@".toCharArray();
	public TouchFilter()
	{
		for( int i=0; i<10; i++)
		{
	...
	public void setMoveRangeMM( float p_fMM )
	{
		...
		m_nMovRange = Math.round( fInch * LayaWrapper.GetDensityDPI());
	}
	/*
	 * 要求：
	 * 	。v必须>0 上限不设。v<0的情况不要用这个函数
	 * 	。返回的字符串不定长，但是最少为2个字节，如果<64会在前面加0
	 */
    protected String code64unsigned(int v)
    {
    	String ret = "";
    	if(v<0)v=0;
    	if( v<64)
    ...
    public Boolean onTouchEvent(MotionEvent event, float kx, float ky)
    {
		...
		if( nPointerCount > 0 )
		{
			...
			case MotionEvent.ACTION_DOWN:
				...
				GL2JNILib.handleTouch( nAction, (id<10)?id:0, nx, ny );
				break;
			...
			case MotionEvent.ACTION_MOVE:
				{
					for( int i = 0; i < nPointerCount; i++ )
					{
						...
						if( !bIgnoreCurEvent )
						{
							GL2JNILib.handleTouch( nAction, ( nID<10 ) ? nID : 0, nx1, ny1 );
						}
					}
				}
				break;
