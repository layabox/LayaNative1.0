...
#include <vector>
#include <stdint.h>
#include "../RenderUtil/JCVBOManager.h"
#include <mutex>
...
namespace laya
{
    ...
    public:
        ...
        perfBarData(int id, int color, const char* pdesc,float scale, float w) {
            m_nID = id;
            m_strDesc = pdesc;
            m_nColor = color;
            m_fScale = scale;
    ...
    private:
        ...
        std::mutex              m_DataLock;
        float barWidth = 1.0f;  //如果为0，则不指定，渲染自己处理。
    };
    ...
    */
    ...
    #define	PERF_UPDATE_DTIME(dataid,var)	            \
		    { auto curtm = tmGetCurms();             \
		      if( var==0)var = curtm;                   \
		      auto dt = (curtm-var);                \
		      JCPerfHUD::updateData(dataid, (float)dt); \
		      var = curtm;}
...
public:
    ...
    */
    void drawAsBarGraph(float* pData, int p_nDataNum, float scale, unsigned int p_nColor, float w);
    void invalidGLRes();
...
public:
    int 					m_nWidth;           //面板的大小
    int 					m_nHeight;          //面板的大小
    int                     m_nScrWidth = 800;  //屏幕大小
    int                     m_nScrHeight = 600;
};
