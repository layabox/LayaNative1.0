...
namespace laya
{
    ...
    bool LoadETCFromMem (BitmapData* pBitmapData,unsigned  char * memData, int size );
    bool LoadPVRV2FromMem (BitmapData* pBitmapData,unsigned  char * memData, int size );
    bool LoadPVRV3FromMem(BitmapData* pBitmapData, unsigned  char * memData, int size);
    bool LoadPVRV3NewFromMem(BitmapData* pBitmapData, unsigned  char * memData, int size);
    ...
    char* BitmapData::premultiplyAlphaData()
    {
        if (!m_bPremultiplyAlpha && m_pImageData)
        {
            for (uint32_t i = 0; i < m_nHeight; i++)
            {
                for (uint32_t j = 0; j < m_nWidth * 4; j += 4)
                {
                    unsigned char*  base = (unsigned char*)(&m_pImageData[4 * i * m_nWidth + j ]);
	...
	bool loadImageMemSync( const char* p_pMem, int p_nLenth, BitmapData& p_bmp ){
		...
		case ImgType_jpeg:
			...
			break;
		case ImgType_png:
			p_bmp.m_pImageData = (char*)ReadPNGFromMem(&p_bmp, (unsigned char*)p_pMem, p_nLenth);
        ...
        case ImgType_gif:
            ...
            break;
        case ImgType_pvrv3:
            return LoadPVRV3NewFromMem(&p_bmp, (unsigned char*)p_pMem, p_nLenth);
            break;
#ifdef TPG
        case ImgType_tpg:
            return LoadTPGFromMem(&p_bmp, (unsigned char*)p_pMem, p_nLenth);
            break;
#endif
        case ImgType_unk:
        default:
	...
	void _AsyncLoadImage(std::shared_ptr<char> p_pBuff, int p_nLenth, imgDecodeCB p_CB){
		...
		if( b ){
            ...
            if (bmp.m_pImageData) bmp.releaseData();
			bmp.m_pImageData = 0;
			p_CB(bmp);
		}
	}
	...
	ImageType getImgType( const char* p_pMem, int p_nLength ){
        ...
        else if (isPvrV3(p_pMem, p_nLength)) return ImgType_pvrv3;
#ifdef TPG
        else if (isTpg(p_pMem, p_nLength)) return ImgType_tpg;
#endif
		return ImgType_unk;
	}
	...
	bool saveAsJpeg(const char* p_pData, ImageBaseInfo& p_Info, const char* p_pszFile ){
		...
		bool ret = saveJpeg(width, height, 24,(char*)pData, 80, p_pszFile);
		if( bpp==32)
			delete [] pData;
    ...
    bool isCompressedFormat(int nFormat)
    {
        ...
        case GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG:
            ...
            break;
        default:
            assert(true && "unsupport format");
            break;
        }
        return false;
    }
    //现在不支持mipmap 假定没有mipmap
    int calcTextureSize(int nFormat, int nWidth, int nHeight)
    {
        ...
        case GL_COMPRESSED_RGB_PVRTC_2BPPV1_IMG:
            return nWidth * nHeight * 0.25;
            break;
        case GL_COMPRESSED_RGBA_PVRTC_2BPPV1_IMG:
            return nWidth * nHeight * 0.25;
            break;
        case GL_COMPRESSED_RGB_PVRTC_4BPPV1_IMG:
            return nWidth * nHeight * 0.5;
            break;
        case GL_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG:
            return nWidth * nHeight * 0.5;
            break;
