...
//------------------------------------------------------------------------------
void JCMemorySurvey::printMemoryStatis( const char* p_sPath,int p_nTexturePoolSize, int p_nAtlasPoolSize,int p_nSoundPoolSize, int p_nImagePoolSize)
{
	statisticsMemory();
	std::string sBuffer = "";
    int nCount = 0;
    ...
    sprintf(sTemp1, "Reserve texture manager size=%5.2fMB,Reserve atlas size=%5.2fMB,Sound size=%5.2fMB,Image no release yet=%d\r\n",
        (float)(p_nTexturePoolSize/1024.0f/1024.0f), (float)(p_nAtlasPoolSize/1024.0f/1024.0f), (float)(p_nSoundPoolSize/1024.0f/1024.0f),p_nImagePoolSize );
    LOGI("%s",sTemp1);
    sBuffer += sTemp1;
...
//------------------------------------------------------------------------------
void JCMemorySurvey::printAll(const char* p_sPath, int p_nTexturePoolSize, int p_nAtlasPoolSize, int p_nSoundPoolSize,int p_nImagePoolSize )
{
    if (m_bEnable)
    {
        //printMemorySurveyInfo(p_sPath);
        printMemoryStatis(p_sPath,p_nTexturePoolSize, p_nAtlasPoolSize, p_nSoundPoolSize, p_nImagePoolSize );
    }
    else
    {
