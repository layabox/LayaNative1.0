...
namespace laya{
	...
	*/
    ...
    {
        std::vector<ResOnReadyCB >	    allOnReadyCB;
        std::vector<ResOnErrCB >	    allOnErrCB;
        std::vector<ResOnReadyCB >	    allOnFreeCB;
		...
		case ready:
        {
            swap(m_allOnReadyCB, allOnReadyCB);
            m_allOnReadyCB.clear();
            m_allOnErrCB.clear();
			int nNum = allOnReadyCB.size();
		...
		case error:
        {
            swap(m_allOnErrCB, allOnErrCB);
            m_allOnErrCB.clear();
            m_allOnReadyCB.clear();
			int nNum = allOnErrCB.size();
			for( int i=0; i<nNum; i++){
				allOnErrCB[i](this,m_nErrNo);
			}
		}
        break;
		...
		case freed:
        {
            swap(m_allOnFreeCB, allOnFreeCB);
            m_allOnFreeCB.clear();
			int nNum = allOnFreeCB.size();
			for(int i=0; i<nNum; i++){
				allOnFreeCB[i](this);
			}
        }
