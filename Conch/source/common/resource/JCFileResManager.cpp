...
namespace laya
{
    ...
    JCFileRes* JCFileResManager::getRes(const std::string &p_pszURL, int p_nConnTimeout , int p_nOptTimeout) {
        std::lock_guard<std::mutex> lock(m_maplock);//下面要添加资源，需要锁。因为失败的话会删除
        JCFileRes* pRes = NULL;
        FileResMap::iterator it = m_ResMap.find(p_pszURL);
        if (it == m_ResMap.end()) {
