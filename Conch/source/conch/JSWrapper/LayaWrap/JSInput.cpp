...
#include "../../JCConch.h"
#ifdef JS_JSC
    #include "../JSInterface/JSC/EJConvertTypedArray.h"
#endif
namespace laya
{
    ...
    void JSInput::onDeviceMotionCallJSFunction(DeviceMotionEvent e)
    {
        ...
        if (!isSupportTypedArrayAPI())
        {
            ...
            JSObjectRef arrayObj = JSValueToObject(ctx, JCScriptRuntime::s_JSRT->m_pJSCmdOtherBufferSharedWithJS.m_pValue,NULL);
    ...
    bool JSInput::activeCall(inputEvent e)
    {
        ...
        else
        {
            ...
            if (e.nType == E_ONTOUCHSTART || e.nType == E_ONTOUCHMOVE)
            {
                ...
                if (JCConch::s_pConchRender)
                {
                    auto pFunction = std::bind(&JCConchRender::onTouchStart, JCConch::s_pConchRender, fTime);
