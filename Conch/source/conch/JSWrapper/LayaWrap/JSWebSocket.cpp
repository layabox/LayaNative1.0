...
namespace laya
{
    ...
    void JSWebSocketDelegate::onMessage(WebSocket* ws, const WebSocket::Data& data)
    {
        ...
        m_pCmdPoster->postToJS(
            std::bind(&JSWebSocket::onSocketMessageCallJSFunctionArrayBuffer, m_js_WebSocket, pDt, data.len, data.isBinary, jswsref));
    }
    ...
    //------------------------------------------------------------------------------
    /*
    void JSWebSocket::onSocketMessageCallJSFunction( std::string p_sEvent )
    {
    m_pJSFunctionOnMessage.Call( p_sEvent.c_str() );
    }
    */
