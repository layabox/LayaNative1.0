...
namespace laya {
    ...
    JSOHOSEditBox::JSOHOSEditBox() {
        ...
        NapiHelper::GetInstance()->handleEditBoxMsg("create", m_tag);
    }
    JSOHOSEditBox::~JSOHOSEditBox() {
        JCMemorySurvey::GetInstance()->releaseClass("OHOSEditBox", this);
        NapiHelper::GetInstance()->handleEditBoxMsg("remove", m_tag);
    }
    ...
    int JSOHOSEditBox::set_Left(int p_nLeft) {
        m_nLeft = p_nLeft;
        NapiHelper::GetInstance()->handleEditBoxMsgNum2("setPos", m_tag, m_nLeft, m_nTop);
        return m_nLeft;
    }
    int JSOHOSEditBox::get_Left() { return m_nLeft; }
    int JSOHOSEditBox::set_Top(int p_nTop) {
        m_nTop = p_nTop;
        NapiHelper::GetInstance()->handleEditBoxMsgNum2("setPos", m_tag, m_nLeft, m_nTop);
        return m_nTop;
    }
    int JSOHOSEditBox::get_Top() { return m_nTop; }
    int JSOHOSEditBox::set_Width(int p_nWidth) {
        m_nWidth = p_nWidth;
        NapiHelper::GetInstance()->handleEditBoxMsgNum2("setSize", m_tag, (int)(m_nWidth * m_nScaleX),
                                                        (int)(m_nHeight * m_nScaleY));
        return m_nWidth;
    }
    int JSOHOSEditBox::get_Width() { return m_nWidth; }
    ...
    int JSOHOSEditBox::get_Height() { return m_nHeight; }
    float JSOHOSEditBox::set_Opacity(float p_Opacity) {
        m_fOpacity = p_Opacity;
        NapiHelper::GetInstance()->handleEditBoxMsgNum1("setOpacity", m_tag, m_fOpacity);
        return m_fOpacity;
    }
    float JSOHOSEditBox::get_Opacity() { return m_fOpacity; }
    const char *JSOHOSEditBox::set_Value(const char *p_sValue) {
        m_sValue = (p_sValue != NULL) ? p_sValue : "";
        NapiHelper::GetInstance()->handleEditBoxMsgStr("setValue", m_tag, m_sValue);
        return m_sValue.c_str();
    }
    const char *JSOHOSEditBox::get_Value() {
        if (JCScriptRuntime::s_JSRT->m_ScriptThread.isStop()) {
            return m_sValue.c_str();
        }
        m_sValue = NapiHelper::GetInstance()->handleEditBoxMsgGetStr("getValue", m_tag);
        return m_sValue.c_str();
    }
    void JSOHOSEditBox::set_Style(const char *p_sStyle) {
        if (p_sStyle == NULL)
            ...
            m_sStyle = p_sStyle;
        NapiHelper::GetInstance()->handleEditBoxMsgStr("setStyle", m_tag, m_sStyle);
    }
    const char *JSOHOSEditBox::get_Style() { return m_sStyle.c_str(); }
    bool JSOHOSEditBox::set_Visible(bool p_bVisible) {
        m_bVisible = p_bVisible;
        NapiHelper::GetInstance()->handleEditBoxMsgBool("setVisible", m_tag, m_bVisible);
        return m_bVisible;
    }
    bool JSOHOSEditBox::get_Visible() { return m_bVisible; }
    void JSOHOSEditBox::setLeft(int p_nLeft) { set_Left(p_nLeft); }
    void JSOHOSEditBox::setTop(int p_nTop) { set_Top(p_nTop); }
    void JSOHOSEditBox::setWidth(int p_nWidth) { set_Width(p_nWidth); }
    void JSOHOSEditBox::setHeight(int p_nHeight) { set_Height(p_nHeight); }
    void JSOHOSEditBox::setOpacity(float p_Opacity) { set_Opacity(p_Opacity); }
    void JSOHOSEditBox::setValue(const char *p_sValue) { set_Value(p_sValue); }
    const char *JSOHOSEditBox::getValue() { return get_Value(); }
    void JSOHOSEditBox::setStyle(const char *p_sStyle) { set_Style(p_sStyle); }
    void JSOHOSEditBox::setVisible(bool p_bVisible) { set_Visible(p_bVisible); }
    void JSOHOSEditBox::focus() {
        JCScriptRuntime::s_JSRT->m_pCurEditBox = this;
        m_bVisible = true;
        NapiHelper::GetInstance()->handleEditBoxMsg("focus", m_tag);
    }
    void JSOHOSEditBox::blur() {
        m_bVisible = false;
        NapiHelper::GetInstance()->handleEditBoxMsgBool("setVisible", m_tag, false);
        JCScriptRuntime::s_JSRT->m_pCurEditBox = NULL;
    }
    void JSOHOSEditBox::setColor(const char *p_sColor) {
        NapiHelper::GetInstance()->handleEditBoxMsgStr("setColor", m_tag, p_sColor);
    }
    void JSOHOSEditBox::setFontSize(int p_nFontSize) {
        m_nFontSize = p_nFontSize;
        NapiHelper::GetInstance()->handleEditBoxMsgNum1("setFontSize", m_tag, (int)(m_nFontSize * m_nScaleY));
    }
    ...
    void JSOHOSEditBox::setCursorPosition(int pos) {
        NapiHelper::GetInstance()->handleEditBoxMsgNum1("setCursorPosition", m_tag, pos);
    }
    ...
    void JSOHOSEditBox::setMaxLength(int p_nMaxLength) {
        ...
        NapiHelper::GetInstance()->handleEditBoxMsgNum1("setMaxLength", m_tag, m_nMaxLength);
    }
    void JSOHOSEditBox::setType(const char *p_sType) {
        ...
        NapiHelper::GetInstance()->handleEditBoxMsgNum1("setType", m_tag, type);
    }
    void JSOHOSEditBox::setRegular(const char *p_sRegular) {
        m_sRegular = p_sRegular;
        NapiHelper::GetInstance()->handleEditBoxMsgStr("setRegular", m_tag, m_sRegular);
    }
    ...
    void JSOHOSEditBox::setMultiAble(bool p_bMultiAble) {
        NapiHelper::GetInstance()->handleEditBoxMsgBool("setMultiAble", m_tag, p_bMultiAble);
    }
    void JSOHOSEditBox::setForbidEdit(bool bForbidEdit) {
        m_bForbidEdit = bForbidEdit;
        NapiHelper::GetInstance()->handleEditBoxMsgBool("setForbidEdit", m_tag, m_bForbidEdit);
    }
    bool JSOHOSEditBox::getForbidEdit() { return m_bForbidEdit; }
