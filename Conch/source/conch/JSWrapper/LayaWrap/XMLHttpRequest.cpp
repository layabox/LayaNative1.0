...
namespace laya
{
    ...
    */
    ...
    void _onPostComplete(XMLHttpRequest* pxhr, bool bBin, IConchThreadCmdMgr* pPoster, 
            JCBuffer& p_Buff, 
            const std::string&, 
            const std::string&,
            int curlr, int httpr, 
            const std::string&,
        std::weak_ptr<int> callbackref) {
        //bool bin = pxhr->isBin();
    ...
    */
    ...
    void XMLHttpRequest::getData(const char* p_sUrl) {
        ...
        if (!pdmgr) {
            ...
            pdmgr->download(p_sUrl, 0, JCDownloadMgr::defProgressFunc, 
                m_funcPostComplete,
                0,
                headers);
        }
    ...
    int XMLHttpRequest::get_timeout() {
        return m_nTimeout;
    }

    void XMLHttpRequest::set_timeout(int tm) {
        m_nTimeout = tm;
    }
    ...
    void XMLHttpRequest::exportJS()
    {
        ...
        JSP_ADD_PROPERTY(timeout, XMLHttpRequest, get_timeout, set_timeout);
