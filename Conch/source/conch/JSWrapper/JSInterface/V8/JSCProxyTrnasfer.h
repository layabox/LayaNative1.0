...
namespace laya{
    ...
    template <typename T> class __TransferToCpp{public:
		static T ToCpp( Local<Value> p_vl ){
			...
			if( !p_vl.IsEmpty() && p_vl->IsObject() ){
				return static_cast<T>(Local<External>::Cast(Handle<Object>::Cast(p_vl)->GetInternalField(0))->Value());
			}else{
				return (T)0;
			}
        }
    ...
    {
    public:static Handle<Value> ToJs(long unsigned int p_vl) { return Integer::New(Isolate::GetCurrent(), p_vl); }
    };
