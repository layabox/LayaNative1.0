...
{
	LOGE(">>>>>>>>>>>>>>>>>>>>>>>>>>>>callMethod1 className:%s,methodName：%s,params: %s", className,methodName,params);
...
{
	LOGE(">>>>>>>>>>>>>>>>>>>>>>>>>>>>callMethod2 className:%s,methodName：%s", className,methodName);
    ThreadJNIData* pThreadData = checkThreadJNI();
    if (!pThreadData)return false;
    JNIEnv* pJNI = pThreadData->pThreadJNI;
    jstring jclassName = pJNI->NewStringUTF(className);
...
bool CToJavaBridge::getTextBitmap(laya::BitmapData* bitmapData, const char*text, laya::JCFontInfo* pFontInfo, int fontColor)
{
	LOGE(">>>>>>>>>>>>getTextBitmap todo delete");
    ThreadJNIData* pThreadData = checkThreadJNI();
    if (!pThreadData)return false;
    JNIEnv* pJNI = pThreadData->pThreadJNI;
    ...
    jobjectArray objectArray = (jobjectArray)pJNI->CallStaticObjectMethod(m_jClass, m_jNativeMethodStrId1, jstrText, jstrFont, pFontInfo->m_nFontSize, fontColor, pFontInfo->m_nBorderSize, pFontInfo->m_nBorderColor);
