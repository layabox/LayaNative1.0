...
namespace cacheTest{
    JCFileRes* createFileRes(const char* pFile, const char* pData, int len) {
        ...
        pRes->m_nLocalFileID = svc.getFileID(pFile);
        pRes->m_nLength = len;
	...
	void testSessionFileGet(){
        ...
        svc.switchToApp("http://localhost/app/");
        //没有dcc的情况下，无法获得资源
        unsigned int fid = svc.getFileID("http://localhost/app/test.png");
...
Content-Encoding: gzip
    ...
    /**
        filetable可以动态改变，不用重建AppCach
    */
    void testUpdateFileTable() {

    }

    /**
        可以指定替换表。即实际url到cache中的文件的转换关系。
        加载cache的时候，要转换
        下载完成更新cache也要转换
    */
    void testDccUrlTrans() {
        class TestDownloadMgr :public JCDownloadMgr {
        public:
            virtual void download(const char* p_pszURL, int p_nPriority,
                onProgressFunc p_ProgCb,
        ...
        svc.setUrlTransTable(R"(
/test_00001.png /test.png
/test1_00002.png /test1.png
)",' ');
        JCFileRes* pFR = new JCFileRes(pDmgr, &fmgr);
        unsigned int id = svc.getFileID("http://localhost/app/test_00001.png");
        pFR->load("http://localhost/app/test_00001.png", nullptr);

        svc.setUrlTransTable(R"(
/test_00002.png /test.png
/test1_00003.png /test1.png
)", ' ');

        pFR->load("http://localhost/app/test_00002.png", nullptr);
    }
