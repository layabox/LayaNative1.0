...
namespace MonkVG {
	...
	OpenGLPaint::~OpenGLPaint() {
		if ( _gradientImage != VG_INVALID_HANDLE ) {
            delete (OpenGLImage*)_gradientImage;
		}
	}
	...
	void OpenGLPaint::buildLinearGradientImage( VGfloat pathWidth, VGfloat pathHeight ) {
		...
		// create openvg image
		_gradientImage = (VGImage)m_pCtx->CreateImage(VG_sRGBA_8888, width, height, 0 );
		
        m_pCtx->ImageSubData(_gradientImage, image, -1, VG_sRGBA_8888, 0, 0, width, height);
    ...
    void OpenGLPaint::buildLinear2x3GradientImage( VGfloat pathWidth, VGfloat pathHeight ) {
        ...
        float gradientSize = 32768 * 0.05f;
