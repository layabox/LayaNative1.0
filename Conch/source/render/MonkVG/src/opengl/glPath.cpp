...
namespace MonkVG {
	...
	bool OpenGLPath::draw( VGbitfield paintModes ) {
		...
		OpenGLContext& glContext = (MonkVG::OpenGLContext&)*m_pCtx;
		
		if( paintModes & VG_FILL_PATH ) {	// build the fill polygons
			buildFillIfDirty();
		}

		if( paintModes & VG_STROKE_PATH && (_isStrokeDirty == true || m_pCtx->currentBatch())  ) {
			buildStroke();
...
#define M_PI 3.14159265358979323846 
	...
	void OpenGLPath::endOfTesselation( VGbitfield paintModes ) {
		...
		if ( _vertices.size() > 0 ) {
            ...
            OpenGLES::OpenGLESContext* pgl = ((OpenGLContext*)m_pCtx)->gl();
			pgl->glGenBuffers( 1, &_fillVBO );
			gl->glBindBuffer( GL_ARRAY_BUFFER, _fillVBO );
			if ( _fillPaintForPath && _fillPaintForPath->getPaintType() == VG_PAINT_TYPE_COLOR ) {
		...
		OpenGLBatch* glBatch = (OpenGLBatch*)m_pCtx->currentBatch();
